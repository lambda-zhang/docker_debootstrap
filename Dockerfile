...
COPY ./install_meteor.sh ./
COPY ./nodejs_setup_8.x ./
...
WORKDIR /root
ADD ./empty_app /root
RUN cd /root/empty_app && meteor build /root/build-hot-share --server=https://hostgst.tiegushi.com/ && rm -rf /root/build-hot-share && rm -rf .meteor/local/
